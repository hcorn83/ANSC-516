...
meta<-readr::read_tsv("metadata_henk.txt")
str(meta)
colnames(meta)[2] <- "Company"
colnames(meta)[3] <- "Store"
colnames(meta)[8] <- "Biofilm"
colnames(meta)[9] <- "State"
str(meta)
...
fdp <- faith_pd[,-1] %>% rename("SampleID" = "V1") %>% rename("Faith_PD" = "V2")

meta <- meta %>% rename("SampleID" = "ID") 
str(meta)
...
alpha_diversity = merge(x=fdp, y=evenness, by.x = "SampleID", by.y = "SampleID")
...
hist(meta$pielou, main="Evenness", xlab="", breaks=10)
hist(as.numeric(meta$observed_features), main="Observed Features", xlab="", breaks=10)

#Plots the qq-plot for residuals
ggqqplot(alpha_diversity$shannon_entropy, title = "Shannon")
ggqqplot(alpha_diversity$Faith_PD, title = "Faith PD")
ggqqplot(alpha_diversity$pielou_e, title = "Evenness")
ggqqplot(alpha_diversity$observed_features, title = "Observed Features")
...
shapiro.test(alpha_diversity$pielou_e)
...
aov.evenness.SampleID = aov(pielou_evenness ~ SampleID, data=alpha_diversity)
#Call for the summary of that ANOVA, which will include P-values
summary(aov.evenness.SampleID)
...
TukeyHSD(aov.evenness.SampleID)
...
levels(alpha_diversity$SampleID)
#Re-order the groups because the default is alphabetical order
meta$State.ord = factor(meta$State, c("A", "B", "C"))
levels(meta$State.ord)

#Plot
boxplot(pielou_evenness ~ State, data=meta, ylab="Peilou evenness")

evenness <- ggplot(meta, aes(State, pielou_evenness)) + 
  geom_boxplot() + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
...
evenness_summary <- meta %>% # the names of the new data frame and the data frame to be summarised
  group_by(State.ord) %>%   # the grouping variable
...
kruskal.test(Faith_PD ~ State.ord, data=meta)
...
pairwise.wilcox.test(meta$Faith_PD, meta$State.ord, p.adjust.method="BH")

# Like evenness, we see that pd also increases with age.

#Plot
boxplot(Faith_PD ~ State.ord, data=meta, ylab="Faith phylogenetic diversity")

# or with ggplot2

observed_features <- ggplot(meta, aes(State.ord, observed_features)) + 
  geom_boxplot(aes(color = State.ord)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Observed Features", x = "") 
ggsave("output/od.png", observed_features, height = 3, width = 3)
