...
str(metadata)
colnames(metadata)[3] = "State"
levels(metadata$State)
metadata$State = factor(metadata$State, c("A", "B", "C"))
levels(metadata$State)
colnames(metadata)[8] <- "Store"
colnames(metadata)[9] <- "Biofilm"
str(metadata)

metadata <- rename(metadata,"SampleID"="ID") 
str(metadata)
...
physeq <- qza_to_phyloseq(
  ...
  tree="rooted-treeC.qza",
  taxonomy = "taxonomy.qza",
  metadata = "metadata_henk.txt"
)
...
for(ml in my_level){
  ...
  physeq_meta_filtered$State.ord = factor(physeq_meta_filtered$State, c("A", "B", "C"))
...
colnames(metadata)[State.1(metadata)]
...
diagdds = phyloseq_to_deseq2(physeq_deseq, ~ State)
...
ggsave(paste0("output/DESeq2-", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_fig, height = 10, width = 10)